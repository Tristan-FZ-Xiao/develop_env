...
def check_file_single(dump_file):
    ...
    for cfg in data.configurations:
        ...
        if len(data.configurations) > 1:
            print('Checking ' + dump_file + ', config "' + cfg.name + '"...')
...
def do_check(arg, dirname, files):
    for file in files:
        ...
        if os.path.isfile(file_path):
            ...
            check_file_single(file_path)

def check_path(path):
    os.path.walk(path, do_check, ())

def generate_dump_files(path):
    import subprocess
    subprocess.Popen("cppcheck --dump " + path)

def misra_check(argv):
    rule_text_file = ''
    loadRuleTexts("rule.txt")
    for arg in argv:
        if arg == '-verify':
            VERIFY = True
        elif ".dump" in arg:
